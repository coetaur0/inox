...
object AliasMap {
  ...
  def init(locals: IndexedSeq[Local]): AliasMap = AliasMap(locals.map { local =>
...
class AliasMap(val bindings: IndexedSeq[(Boolean, AliasSet)]) {
  ...
  /** Returns a new alias map where the binding for a specific local id has been updated to a new
    * set of aliases.
    */
  def updated(id: LocalId, aliases: AliasSet): AliasMap =
    AliasMap(bindings.updated(id, (bindings(id)._1, aliases)))

  /** Returns a new alias map where the bindings for a set of local ids have been updated to a new
    * set of aliases.
    */
  def updated(ids: Set[LocalId], aliases: AliasSet): AliasMap = AliasMap(
    bindings.zipWithIndex.map((info, index) =>
  ...
  /** Alias for `appended`. */
  def :+(mutable: Boolean, aliases: AliasSet): AliasMap = appended(mutable, aliases)

  /** Alias for `union`. */
  def |(that: AliasMap): AliasMap = union(that)

  /** Returns a new alias map where a new binding has been appended. */
  def appended(mutable: Boolean, aliases: AliasSet): AliasMap = AliasMap(
    bindings :+ (mutable, aliases)
  )

  /** Returns the union of two alias maps. */
  def union(that: AliasMap): AliasMap = AliasMap(
