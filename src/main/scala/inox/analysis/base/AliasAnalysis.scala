...
import inox.analysis.base.AliasMap
...
object AliasAnalysis {
  ...
  def operandAliases(aliases: AliasMap, operand: Operand): AliasSet = operand.item match {
  ...
  def placeAliases(aliases: AliasMap, place: PlaceKind): AliasSet = place match {
  ...
  /** Returns the set of local ids that the result of a call expression may alias. */
  @tailrec
  private def resultAliases(
      ...
      resultType: Type,
      arg: Operand
  ): AliasSet = (paramType.value.item, resultType.value.item) match {
    case (TypeKind.Ref(_, _, _), TypeKind.Ref(_, _, rType)) =>
      if (paramType :< resultType) {
        operandAliases(aliases, arg)
      } else {
  ...
  private def initExtern(
      ...
      name: Name,
      ty: Type
  ): (IndexedSeq[Local], AliasMap) = ty.value.item match {
    case TypeKind.Ref(_, mut, rType) => {
      ...
      (newLocals :+ Local(mutable, name, ty), newAliases :+ (mut, Set(newLocals.length - 1)))
    }
    case _ => (locals :+ Local(mutable, name, ty), aliases :+ (false, Set.empty))
  }
...
private class AliasAnalysis(module: inox.ir.Module, locals: IndexedSeq[Local])
  ...
  override def callInstr(
    ...
    operandType(callee).value.item match {
      case TypeKind.Fn(params, result) => {
        val aliasSet: AliasSet = params.zip(args).foldLeft(Set.empty) { (aliases, arg) =>
  ...
  override def binaryInstr(
      ...
      lhs: Operand,
      rhs: Operand
  ): IndexedSeq[AliasMap] = states :+ states.last
