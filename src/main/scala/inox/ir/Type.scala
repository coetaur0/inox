...
case class Type(value: Spanned[TypeKind]) {
  ...
  def ===(that: Type): Boolean = (this.value.item, that.value.item) match {
    case (Fn(lParams, lResult), Fn(rParams, rResult)) =>
      lParams.length == rParams.length &&
      lParams.zip(rParams).forall { (lParam, rParam) =>
        lParam === rParam
      } &&
      lResult === rResult
  ...
  /** Substitutes the origin ids in a type with the ids in some substitution map. */
...
object Type {
  ...
  def Ref(
      ...
      ty: Type,
      span: Span
  ): Type = Type(Spanned(TypeKind.Ref(origin, mutable, ty), span))
