...
object Lowerer {
  ...
  /** Returns a mapping from function names to their declared origin ids and type. */
  private def getGlobals(moduleDecl: ModuleDecl): Result[Globals, LoweringError] = Result.build {
    errors =>
      val globals = Map.newBuilder[String, (origins: OriginIds, ty: Type)]
      for { (name, function) <- moduleDecl } {
  ...
  /** Returns a mapping from origin names to their ids for a given function declaration. */
  private def getOriginIds(fnDecl: FnDecl): Result[OriginIds, LoweringError] =
    Result.build { errors =>
      val originIds = Map.newBuilder[String, OriginId]
      for { (origin, index) <- fnDecl.origins.zipWithIndex } {
        if (originIds.result().contains(origin.item)) {
          errors += DuplicateOrigin(origin)
        } else {
          originIds += ((origin.item, index))
        }
      }
      originIds.result()
    }
  ...
  private def lowerTypeExpr(origins: OriginIds, ty: TypeExpr): Result[Type, LoweringError] = {
  ...
  private def lowerFnType(
    ...
    lowerTypeExpr(origins, result) match {
      case Result.Success(ty)   => Type.Fn(paramTypes.result(), ty, span)
  ...
  private def lowerOrigin(
      origins: OriginIds,
      origin: Option[Name]
  ): Result[Option[OriginId], LoweringError] = origin match {
    case Some(name) =>
      if (origins.contains(name.item)) {
        Result.Success(Some(origins(name.item)))
      } else {
        Result.fail(UndefinedOrigin(name))
      }
...
private class Lowerer(globals: Globals) {
  ...
  private def lowerFunction(name: String, fnDecl: FnDecl): Result[Function, LoweringError] = {
    originIds = globals(name).origins
    localIds.clear()
    locals.clear()
    Result.build { errors =>
      Lowerer.lowerTypeExpr(originIds, fnDecl.result) match {
        case Result.Success(ty)   => locals += Local(true, Spanned("ret", fnDecl.result.span), ty)
        case Result.Failure(errs) => errors ++= errs
      }
      for { param <- fnDecl.parameters } {
        if (!(localIds += (param.name.item, locals.length))) {
          errors += DuplicateParameter(param.name)
        } else {
          Lowerer.lowerTypeExpr(originIds, param.ty) match {
            case Result.Success(ty)   => locals += Local(param.mutable, param.name, ty)
            case Result.Failure(errs) => errors ++= errs
          }
        }
      }
  ...
  private def lowerStmt(stmt: Stmt): Result[Block, LoweringError] = stmt.item match {
    case ast.StmtKind.While(cond, body)             => lowerWhile(cond, body.item)
    case ast.StmtKind.Let(mutable, name, ty, value) => lowerLet(mutable, name, ty, value)
    case ast.StmtKind.Assign(lhs, rhs)              => lowerAssignment(lhs, rhs)
    case ast.StmtKind.Return(value)                 => lowerReturn(value)
    case ast.StmtKind.ExprStmt(kind)                => lowerExpr(Spanned(kind, stmt.span)).map(_._1)
  }
    ...
    (ty.map(t => Lowerer.lowerTypeExpr(originIds, t)), value.map(v => lowerExpr(v))) match {
      case (Some(t), Some(v)) =>
        for {
          ...
          block :+ Instr.Assign(Place.Var(locals.length - 1, name.span), operand)
        }
      case (Some(t), None) =>
        for { ty <- t } yield {
          locals += Local(mutable, name, ty)
      ...
      case (None, Some(v)) =>
        for { (block, operand, ty) <- v } yield {
          locals += Local(mutable, name, ty)
          localIds += (name.item, locals.length - 1)
          block :+ Instr.Assign(Place.Var(locals.length - 1, name.span), operand)
        }
      case (None, None) => Result.fail(UndefinedType(name))
    }
  ...
  private def lowerAssignment(lhs: Expr, rhs: Expr): Result[Block, LoweringError] = for {
    ...
    result <- lhsOperand.item match {
      ...
      case _ => Result.fail(UnassignableExpr(lhs.span))
    }
  } yield {
    result
  }
  ...
  private def lowerReturn(value: Expr): Result[Block, LoweringError] =
    for { (block, operand, _) <- lowerExpr(value) } yield {
      (block :+ Instr.Assign(Place.Var(0, value.span), operand)) :+ Instr.Return
    }

  /** Lowers an expression to its IR representation. */
  private def lowerExpr(expr: Expr): Result[(Block, Operand, Type), LoweringError] =
    expr.item match {
      ...
      case ExprKind.IntLit(value) =>
        Result.Success((IndexedSeq(), Operand.I32(value, expr.span), Type.I32(expr.span)))
      case ExprKind.BoolLit(value) =>
        Result.Success((IndexedSeq(), Operand.Bool(value, expr.span), Type.Bool(expr.span)))
      case ExprKind.Unit =>
        Result.Success((IndexedSeq(), Operand.Unit(expr.span), Type.Unit(expr.span)))
    }

  /** Lowers a block expression to its IR representation. */
  private def lowerBlock(block: BlockExpr): Result[(Block, Operand, Type), LoweringError] =
    Result.build { errors =>
      val blockBuilder = IndexedSeq.newBuilder[Instr]
      localIds.push(true)
      for { stmt <- block.stmts } {
        lowerStmt(stmt) match {
          case Result.Success(item) => blockBuilder ++= item
          case Result.Failure(errs) => errors ++= errs
        }
      }
      val (operand, ty) = lowerExpr(block.result) match {
        case Result.Success((instrs, operand, ty)) => {
          blockBuilder ++= instrs
          (operand, ty)
        }
        case Result.Failure(errs) => {
  ...
  private def lowerCall(
    ...
    result <- Result.build { (errors: mutable.Builder[LoweringError, Seq[LoweringError]]) =>
  ...
  private def lowerBinary(
    ...
    val ty =
      op match {
        case BinaryOp.Add | BinaryOp.Sub | BinaryOp.Mul | BinaryOp.Div => Type.I32(span)
  ...
  private def lowerDeref(expr: Expr, span: Span): Result[(Block, Operand, Type), LoweringError] =
    lowerExpr(expr).flatMap { case (block, operand, ty) =>
      (operand.item, ty.value.item) match {
        case (OperandKind.Place(place), TypeKind.Ref(_, _, rType)) =>
          Result.Success(
            (block, Operand.Place(PlaceKind.Deref(Spanned(place, expr.span)), span), rType)
          )
        case (_, _) => Result.fail(InvalidDeref(ty))
      }
    }
  ...
  private def lowerVar(
      ...
      span: Span
  ): Result[(Operand, Type), LoweringError] = localIds(name.item) match {
    case Some(id) =>
      Result.Success((Operand.Place(PlaceKind.Var(id), span), locals(id).ty))
    case None =>
      globals.get(name.item) match {
        case Some((_, ty)) =>
          for { args <- lowerOriginArgs(origins) } yield {
            (Operand.Fn(name, args, span), ty.substitute(args))
          }
        case None => Result.fail(UndefinedName(name))
      }
  }
  ...
  ): Result[IndexedSeq[Option[OriginId]], LoweringError] = Result.build { errors =>
  ...
  private def asPlace(operand: Operand, ty: Type): (Block, Place) = operand.item match {
    case OperandKind.Place(place) => (IndexedSeq(), Spanned(place, operand.span))
    case _                        => {
