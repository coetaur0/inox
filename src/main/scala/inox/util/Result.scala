...
object Result {
  ...
  def build[Item, Error](f: mutable.Builder[Error, Seq[Error]] => Item): Result[Item, Error] = {
    ...
    if (errorBuilder.result().isEmpty) {
      ...
      Failure(errorBuilder.result())
    }
  }

  /** Instantiates a new failure with a given error. */
  def fail[Item, Error](error: Error): Result[Item, Error] = Failure(IndexedSeq(error))
}

/** A compilation result. */
enum Result[Item, Error] {
  case Success(item: Item)
  case Failure(errors: Seq[Error])
  ...
  def map[B](f: Item => B): Result[B, Error] = this match {
    case Success(item)   => Success(f(item))
    case Failure(errors) => Failure(errors)
  }

  def flatMap[B](f: Item => Result[B, Error]): Result[B, Error] = this match {
    case Success(item)   => f(item)
    case Failure(errors) => Failure(errors)
  }

  /** Applies some side effect function on the error values in a result if it is a failure. */
  def handleFailure(f: Seq[Error] => Unit): Unit = this match {
    case Success(_)      => ()
