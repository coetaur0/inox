...
import inox.util.{Location, Span, Spanned}
...
class Lexer(source: String) {
  ...
  /** Peeks at the character at some offset from the lexer's current location in the source.
    */
  private def peek(n: Int): Option[Char] = if (offset + n < source.length) {
    Some(source(offset + n))
  } else {
    None
  }

  /** Advances the lexer's location in the source to the next character. */
  private def advance(): Unit = if (offset < source.length) {
    if (source(offset) == '\n') {
      line += 1
      column = 1
    } else {
  ...
  private def advance(n: Int): Unit = for (i <- 1 to n) {
    advance()
  }

  /** Consumes characters from the source as long as they satisfy some `predicate` and returns them.
    */
