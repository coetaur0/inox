...
/** An expression. */
object Expr {
  def Block(body: BlockExpr, span: Span): Expr = Spanned(ExprKind.Block(body), span)
  ...
  def Unary(op: UnaryOp, expr: Expr, span: Span): Expr = Spanned(ExprKind.Unary(op, expr), span)

  def Var(name: Name, origins: IndexedSeq[Option[Name]], span: Span): Expr =
    Spanned(ExprKind.Var(name, origins), span)

  def IntLit(value: Int, span: Span): Expr = Spanned(ExprKind.IntLit(value), span)
